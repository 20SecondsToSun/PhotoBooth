...
void Draw::circleSlice(float x,float y, float radius, float lowAngle, float highAngle, bool closed, bool radians)
{
	...
	if(!radians){  
		lowAngle = toRadians(lowAngle);  
		highAngle = toRadians(highAngle);  
	}  
